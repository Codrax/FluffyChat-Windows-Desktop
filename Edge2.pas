...
function TCustomEdgeBrowser.ClearVirtualHostNameToFolderMapping(
  HostName: string): boolean;
begin
  Result := False;
  if FWebView3 <> nil then
  begin
    Result := ProcessHResult(FWebView3.ClearVirtualHostNameToFolderMapping(PChar(HostName)));
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Supports(FWebView, ICoreWebView2_3, FWebView3);
      Supports(FWebView, ICoreWebView2_2, FWebView2);
...
begin
  ...
  if FWebView2 <> nil then
  begin
    Result := ProcessHResult(FWebView2.NavigateWithWebResourceRequest(ARequest));
  end;
end;
...
function TCustomEdgeBrowser.SetVirtualHostNameToFolderMapping(HostName,
  FolderPath: string;
  AccessKind: COREWEBVIEW2_HOST_RESOURCE_ACCESS_KIND): boolean;
begin
  Result := False;
  if FWebView3 <> nil then
  begin
    Result := ProcessHResult(FWebView3.SetVirtualHostNameToFolderMapping(
      PChar(HostName), PChar(FolderPath), AccessKind));
  end;
end;
